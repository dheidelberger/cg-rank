  ...
  <v-container>
    <v-row class="text-center" justify="center">
      <v-col class="mb-2" cols="8">
        <h1 class="display-2 font-weight-bold mb-3 amber--text accent-3">Playground</h1>
        ...
        <p class="subheading font-weight-regular">
          ...
          >Rankings shown in the playground are sample data.</span>
        </p>
      </v-col>
    </v-row>
    <v-row class="mt-0">
      <v-col cols="12" class="mt-0">
        <v-col cols="12">
          <h4 class="pt-1">Sample Template:</h4>
        </v-col>
        <v-textarea
      ...
      <v-col cols="12">
        <v-col cols="12">
          <h4 class="pt-1">Sample Output:</h4>
        </v-col>
        <v-card>
          <v-card-text class="text-h5">{{ parseTemplate(templateText) }}</v-card-text>
        </v-card>
      </v-col>
    </v-row>
    <v-row justify="center">
      <v-col cols="12">
        <v-sheet elevation="6">
          <v-col cols="12">
            <h4 class="pt-3">Available Tags</h4>
          </v-col>
          ...
          <v-simple-table fixed-header height="400px">
            ...
            <tbody>
              <tr class="text-left" v-for="key in Object.keys(tags)" :key="tags[key].tag">
    ...
    <v-snackbar v-model="snackbar" :color="snackbarColor" :timeout="snackbarTime" dark>
      {{ snackbarText }}
  ...
  methods: {
    parseTemplate(templateText) {
      ...
      for (let tag of tagArray) {
        ...
        outText = outText.replace(thisRX, (match, p1, p2, p3) => {
          const newText = tag.transform(myData, JSON.stringify([p1, p2, p3]));
    ...
    copyItem: function(item) {
      ...
      this.$copyText(vm.mustache(item.tag)).then(
        ...
        function() {
          vm.snackbarText = `Error: Could not copy ${vm.mustache(item.tag)}`;
